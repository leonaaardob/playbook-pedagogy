...
Produire le playbook pédagogique global de la plateforme **LBFrame / academy.lbframe.com**, applicable à l’ensemble du parcours de six cours. Le cours 01 (« Primitives de l’agentic engineering ») est un cas d’application documenté ; il ne doit ni dicter mécaniquement la structure de tous les cours ni être confondu avec le playbook global.
...
2. **Carte du parcours** : rôle, frontières, dépendances et renvois entre les six cours. Éviter les recouvrements : primitives → context → loop → graph → harness → production.
...
Le playbook doit fournir des paramètres explicites, plutôt que rendre implicites les choix du cours 01 : prérequis, profondeur du module, artefact du workshop, niveau de guidage, forme de validation, accès à un modèle, budget, contraintes de sécurité et critères de sortie.

## Points à laisser visibles comme décisions à confirmer

- Le PRD historique parle de sept cours alors que le plan inter-cours actuel en définit six.
